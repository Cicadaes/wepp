...
var dispatch = require('./master_dispatch');
...
Master.prototype = new (function () {
    ...
    this.addWorkListeners = function (worker) {
        var self = this;
    ...
    this.killWorker = function (pid) {
        ...
        var self = this,
            id = pid.toString(),
            work = this.worker.getItem(id).worker;

        worker.killed = true;
        process.kill(pid);
        setTimeout(function () {
            self.handleWorkExit(pid);
        }, 30000);
    }

    this.handleWorkExit = function (pid) {
        var id = pid.toString(),
