...
var app = function () {
    ...
    this.start = function (next) {
        ...
        wepp.worker.server.addListener('request', function (req, res) {
            ...
            domainInst.on('error', function (err) {
                // caught e.g, `throw new errors.NotFoundError(reqUrl + ' not found.')`
                var serverErr;
                var controllerInst;
                    ...
                    if (controllerInst) {
                        ...
                        if (typeof controllerInst[params.action] == 'function') {
                            self.handleControllerAction(controllerInst, reqUrl, method,
                                params, accessTime, reqObj, resObj);
                        } else {
                            // No action, 500 error
    ...
    this.handleControllerAction = function (controllerInst, reqUrl, method,
        params, accessTime, reqObj, resObj) {
            ...
            // Mix all the shits onto controller instance
            ...
            resObj.controller = controllerInst;
